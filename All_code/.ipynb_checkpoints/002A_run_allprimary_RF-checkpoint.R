...
Tumor_preds = fread(pff("data/001G_PrimaryTumor_preds_1MB.csv"))[,-c(1,2)]
RT = fread(pff("data/001F_ENCODE_repliseq_1MBwindow_processed.csv"))[,-c(1,2)]
Preds = as.data.table(cbind.data.frame(Tumor_preds, RT))
...
#Save results
fwrite(RF_result, paste0(pff("/data/002A_primarytumour_RF_Results/"), project_code, ".csv"))
