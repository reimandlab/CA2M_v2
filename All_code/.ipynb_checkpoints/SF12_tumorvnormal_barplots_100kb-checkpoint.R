...
cohort_names = unlist(lapply(list.files(pff("data/SF10_tumorvnormal_100KB/Tumor/")), 
										function(x) unlist(strsplit(x, 
																	split = ".csv", 
																	fixed = T))[1]))

#Load in tumor CA + all RT random forest results
data_paths_tumor_RT = list.files(pff("data/SF10_tumorvnormal_100KB/Tumor/"), full.names = T)
...
data_paths_normal_RT = list.files(pff("data/SF10_tumorvnormal_100KB/Normal/"), full.names = T)
accuracies_normal_RT = unlist(lapply(data_paths_normal_RT, 
								 function(x) as.numeric(fread(x)[["Test_adj_R2"]])))

#Combine results into one data table
accuracies.dt = as.data.table(cbind.data.frame(cohort_name = unlist(lapply(cohort_names,rep, 100)), 
...
get_emp_pval = function(cohort_name_in){
	...
	p_val  = sum(sign(data$diff_adjR2) != sign(mean_diff))/100
	return(p_val)
...
#Create barplot
cairo_pdf(pff("data/SF12_tumourvsnormal_barplots_100KB.pdf"), width = 9, height = 5)                       
ggplot(plot_dt, 
