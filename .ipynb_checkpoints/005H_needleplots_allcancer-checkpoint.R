...
get_100bp_data = function(cohort_name){
	...
	get_pred_data = function(top_pred){
		...
		if(top_pred %in% importances[[1]][1:381]){
		...
		}else if(top_pred %in% importances[[1]][382:584]){
		...
		#If predictor is in RT
		}else if(top_pred %in% importances[[1]][772:867]){

			path = RT_paths[which(importances[[1]][772:867] == top_pred)]

			BigWig = import(path)

			#Liftover to hg38
			BigWig = unlist(liftOver(BigWig, ch))
		}else if(top_pred %in% importances[[1]][700:754]){
			...
			top_pred = predictor_descriptions$Predictor_descriptions[match(top_pred, predictor_descriptions$Predictor_names)]	
		}
...
cancer_types_to_check = colnames(q_val_dt)[-c(1,2)]						  
